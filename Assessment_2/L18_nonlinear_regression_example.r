...
nlreg2 <- nls(y~exp(b*x), start=list(b=.02))
cc = coef(nlreg2)
curve(exp(cc[1]*x), col='brown', add=T, lwd=3) 
legend("topleft", col=c('black','orange','blue','brown'), lwd=3,bty='n',
...
is = order(x)
lines(x[is], fitted(nlso)[is], pch=20, col=4, lwd=5)
